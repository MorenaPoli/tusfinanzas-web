  ...
  if (step === 0) {
    return (
      <div className="max-w-lg mx-auto px-6 pt-6">
        ...
        <p className="text-sm text-white/40 mb-6">Que tipo de movimiento queres registrar?</p>
  ...
  return (
    <div className="max-w-lg mx-auto px-6 pt-6">
      ...
      <div className="space-y-5">
        <div>
          <label className="text-[10px] text-white/30 uppercase tracking-wider mb-2 block">Monto</label>
          <div className="relative">
            <span className="absolute left-4 top-1/2 -translate-y-1/2 text-white/30 text-lg">$</span>
            <input type="number" value={amount} onChange={e => setAmount(e.target.value)} placeholder="0.00"
              className="w-full pl-10 pr-5 py-4 bg-[#1A1A1A] border border-white/[0.08] rounded-2xl text-lg text-white placeholder:text-white/20 focus:outline-none focus:border-[#8B5CF6] transition-colors"
              autoFocus />
          </div>
        </div>

        <div>
          <label className="text-[10px] text-white/30 uppercase tracking-wider mb-2 block">Divisa</label>
              ...
              <button key={curr} type="button" onClick={() => setCurrency(curr)}
                className={`px-4 py-2 rounded-xl text-xs font-semibold border transition-all ${
                  currency === curr
                    ...
                    : 'bg-[#1A1A1A] border-white/[0.08] text-white/60 hover:bg-white/5'
                }`}>
                {curr}
              </button>
            ))}
        ...
        <div>
          <label className="text-[10px] text-white/30 uppercase tracking-wider mb-2 block">Categoria</label>
        ...
        <div>
          <label className="text-[10px] text-white/30 uppercase tracking-wider mb-2 block">Descripcion (opcional)</label>
          <input type="text" value={description} onChange={e => setDescription(e.target.value)} placeholder="Ej: Salario enero"
            className="w-full px-5 py-4 bg-[#1A1A1A] border border-white/[0.08] rounded-2xl text-sm text-white placeholder:text-white/20 focus:outline-none focus:border-[#8B5CF6] transition-colors" />
        </div>

        <div>
          <label className="text-[10px] text-white/30 uppercase tracking-wider mb-2 block">Fecha</label>
          <input type="date" value={date} onChange={e => setDate(e.target.value)}
            className="w-full px-5 py-4 bg-[#1A1A1A] border border-white/[0.08] rounded-2xl text-sm text-white focus:outline-none focus:border-[#8B5CF6] transition-colors" />
        </div>

        <motion.button whileTap={{ scale: 0.97 }} onClick={handleSubmit} disabled={!amount || !category || create.isPending}
